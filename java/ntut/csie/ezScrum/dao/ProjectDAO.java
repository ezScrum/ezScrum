...
public class ProjectDAO extends AbstractDAO<ProjectObject, ProjectObject> {
	...
	private long createScrumRole(long projectId, RoleEnum role, ScrumRole scrumRole) {
		IQueryValueSet valueSet = new MySQLQuerySet();
	...
	public ScrumRole getScrumRole(long id, String projectName, RoleEnum role) {
		...
		try {
			if (result.first()) {
				return getScrumRole(projectName, role.name(), result);
			}
	...
	private ScrumRole getScrumRole(String projectName, String role, ResultSet result) {
	...
	private ProjectObject convert(ResultSet result) {
		ProjectObject project = null;
        try {
