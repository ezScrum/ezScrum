...
public class SprintBacklogWebService extends ProjectWebService{
	...
	public SprintBacklogWebService( String username , String userpwd  , String projectID ,int iteration )throws LogonException{
		super(username, userpwd ,projectID );
//		sprintBacklog = new SprintBacklogMapper( super.getProjectList().get( 0 ) , new UserSession( super.getAccount() ) , iteration ) ;
		this.sprintBacklogLogic = new SprintBacklogLogic( super.getProjectList().get( 0 ), new UserSession( super.getAccount() ), String.valueOf(iteration) ) ;
		this.sprintBacklog = this.sprintBacklogLogic.getSprintBacklogMapper();
	}
	public SprintBacklogWebService( String username , String userpwd  , String projectID )throws LogonException{
		super(username, userpwd ,projectID );
//		sprintBacklog = new SprintBacklogMapper( super.getProjectList().get( 0 ) , new UserSession( super.getAccount() ) ) ;
		this.sprintBacklogLogic = new SprintBacklogLogic( super.getProjectList().get( 0 ), new UserSession( super.getAccount() ), null ) ;
		this.sprintBacklog = this.sprintBacklogLogic.getSprintBacklogMapper();
	}
...
//		return csb.readStoryIDList( sprintBacklog );
	...
	public String getTaskIDList(String storyID) throws JSONException {
		ConvertSprintBacklog csb = new ConvertSprintBacklog();
		IIssue[] taskIDList = sprintBacklog.getTasksMap().get( Long.parseLong(storyID) );
		String taskIDListJsonString = csb.convertTaskIDList( storyID, taskIDList );
	...
	public String getTaskHsitoryList(String taskID) throws JSONException {
		ConvertSprintBacklog csb = new ConvertSprintBacklog();
		IIssue taskByID = this.sprintBacklogLogic.getTaskById( Long.parseLong(taskID) );

		List<IIssueHistory> taskHistoryList = taskByID.getIssueHistories();
		List<String> remainsList = new ArrayList<String>();
		String lastRemainsHour = "0";
		for( IIssueHistory history:taskHistoryList ){
			String date = parseDate( history.getModifyDate() );
			lastRemainsHour = getTaskRemains( new Date(date), taskByID );
			remainsList.add( lastRemainsHour );
//			System.out.println("date = " + parseDate(history.getModifyDate()) + ", Remains = " + lastRemainsHour );
		}
		String taskHistoryJsonString = csb.convertTaskHistory( taskHistoryList, remainsList );
	...
	private String getTaskRemains(Date date, IIssue task) {
		...
		try {
			point = Double.parseDouble(task.getTagValue(ScrumEnum.REMAINS, date));
		} catch (Exception e) {
			try {
				// 表示這個Task沒有REMAINS，那麼就取得ESTIMATION八
				point = Double.parseDouble(task.getTagValue(ScrumEnum.ESTIMATION, date));
			} catch (Exception e1) {
				// 如果還是沒有，那就回傳 0
				return "0.0";
			}
	...
	public String getSprintBacklog( String handlerID ) throws JSONException{
		ConvertSprintBacklog csb = new ConvertSprintBacklog();
		return csb.convertStoryTaskInformationList( this.sprintBacklogLogic, this.sprintBacklog, handlerID );
