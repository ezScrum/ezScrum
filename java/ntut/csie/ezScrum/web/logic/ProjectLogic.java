...
//		List<IAccount> projectMemberList = accountMapper.getProjectMemberList(userSession, project);
	...
	public boolean projectIsExistedInWorkspace( String projectID ){
		List<IProject> projects = this.getAllProjectList();
...
//				System.out.println( PID );
	 ...
	 * @return
	 */
	public Map<String, Boolean> getProjectPermissionMap(IAccount account) {
...
//		IPermission permAdmin = manager.getPermission(this.system_Admin);
		...
		if (account.checkPermission(permAdmin)) {
			Iterator<IProject> ir = list.iterator();
	...
	public IProject[] getAllCustomProjects() {
		List<IProject> CustomProjects = new ArrayList<IProject>();	// 可以讓使用者回報 issue 的專案
		List<IProject> projects = getAllProjectList();
	...
	/**
	 * 檢查 metadata 有需要初始化的檔案，複製一份到專案設定檔內。
	 */
	...
	public void check_default_role() {
		List<IProject> projectlist = this.getAllProjectList();
