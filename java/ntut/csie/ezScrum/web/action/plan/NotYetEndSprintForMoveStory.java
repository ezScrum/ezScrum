...
public class NotYetEndSprintForMoveStory extends PermissionAction {
	...
	private String setSprintToJSon(SprintPlanHelper sprintPlanHelper) {
		Date today = new Date();
		StringBuilder sprintTree = new StringBuilder("");

		ArrayList<SprintObject> sprints = sprintPlanHelper.getSprints();
		if (sprints != null) { // 有 sprint 資訊，則抓取 sprint 的 xml 資料
			sprintTree.append("[");
			// 將資訊設定成 JSon 輸出格式 sprint已經開始的無法被移動
			for (int i = 0; i < sprints.size(); i++) {
				// 尚未開始的sprint
				if (stringToDate(sprints.get(i).getEndDateString()).compareTo(today) >= 0) {
