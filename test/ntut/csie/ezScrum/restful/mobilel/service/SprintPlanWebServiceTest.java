...
public class SprintPlanWebServiceTest {
	...
	public void testGetAllSprints() throws Exception {
		...
		for (int i = 0; i < mSprintCount; i++) {
			...
			assertEquals(sprints.get(i).getTeamSize(), sprintJSONObject.get(SprintUtil.TAG_TEAM_SIZE));
	...
	public void testGetCurrentSprint() throws LogonException, JSONException {
		...
		assertEquals(SprintObject.get(mCS.getSprintsId().get(0)).getTeamSize(), sprintJSONObject.get(SprintUtil.TAG_TEAM_SIZE));
