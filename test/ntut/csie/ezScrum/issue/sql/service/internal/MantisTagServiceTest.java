...
public class MantisTagServiceTest {
	...
	public void testInitTag(){
		...
		ArrayList<TagObject> tags = mProductBacklogMapper.getTagList();// 從 sql 取tag list 
		
		assertEquals(story.getTags().size(), tags.size());// TagCount = 2
		for(int listIndex = 0; listIndex < tags.size(); listIndex++){
			assertEquals(story.getTags().get(listIndex).getId(), tags.get(listIndex).getId());// TagID = 1,2...
			assertEquals(story.getTags().get(listIndex).getName(), tags.get(listIndex).getName());// TagName = TEST_TAG_1,2...
		}
	}
	...
	public void testUpdateTag(){
		...
		ArrayList<TagObject> tags = mProductBacklogMapper.getTagList();
		// check update 之前的資料正確與否
		for(int listIndex = 0; listIndex < tags.size(); listIndex++){
			assertEquals(listIndex+1, tags.get(listIndex).getId());// TagID = 1,2...
			assertEquals(TEST_TAG_NAME+Integer.toString(listIndex+1),// TagName = TEST_TAG_1,2... 
					tags.get(listIndex).getName());
		}
		
		// update tag name
		for(int listIndex = 0; listIndex < tags.size(); listIndex++){
			mMantisTagService.updateTag(tags.get(listIndex).getId(),
								"TEST_UPDATE_TAG_"+Integer.toString(listIndex+1),mCP.getProjectList().get(0).getName());
		}

		// check update 之後 ezscrum_tag_table 的資料正確與否
		tags = mProductBacklogMapper.getTagList();
		for(int listIndex = 0; listIndex < tags.size(); listIndex++){
			assertEquals("TEST_UPDATE_TAG_"+Integer.toString(listIndex+1), tags.get(listIndex).getName());
		}
	}
	...
	public void testDeleteTag(){
		...
		// check 預設有兩筆 tag
		ArrayList<TagObject> tags = mProductBacklogMapper.getTagList();
		assertEquals(TagCount, tags.size());//TagCount = 2
		long deleteTagId = tags.get(0).getId();
		...
		tags = mProductBacklogMapper.getTagList();
		assertEquals(TagCount-1, tags.size());//TagCount-1 = 1
	...
	public void testGetTagByName(){
		...
		TagObject resultTag;
		for(int listIndex = 0; listIndex < tags.size(); listIndex++){
			resultTag = mMantisTagService.getTagByName(tags.get(listIndex).getName(), projectName);
			assertEquals(tags.get(listIndex).getId(), resultTag.getId());
			assertEquals(tags.get(listIndex).getName(), resultTag.getName());
		}
	}
	...
	public void testGetTagList(){
		...
		// get tag list from sql
		String projectName = mCP.getProjectList().get(0).getName();
		ArrayList<TagObject> resultTags = mMantisTagService.getTagList(projectName);
		assertEquals(TagCount, resultTags.size());
		// check each tag's info. correct or not.
		for(int listIndex = 0; listIndex < resultTags.size(); listIndex++){
			assertEquals(Integer.toString(listIndex+1), Long.toString(resultTags.get(listIndex).getId()));
			assertEquals(TEST_TAG_NAME+Integer.toString(listIndex+1), resultTags.get(listIndex).getName());// TEST_TAG_X
		}
	}
