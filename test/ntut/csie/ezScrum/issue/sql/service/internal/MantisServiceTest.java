...
//		story.setAssignto("administrator");
	...
	/**
	 * 主要測試getIssue方法中
	 * m_attachFileService.initAttachFile(issue);對getIssue造成的效果是否有效
	 */
	...
	/**
	 * 主要測試getIssues方法中 p.s.這裡是複數s
	 * m_attachFileService.initAttachFile(issue);對getIssues造成的效果是否有效
	 */
	...
	/**
	 * 主要測試getStorys方法中
	 * m_attachFileService.initAttachFile(story);對getStorys造成的效果是否有效
	 */
	public void testGetStorys_AboutAttachFile(){
		List<IStory> story_list = new ArrayList<IStory>();
		//test case:測試初始狀態 story_list size 為 0
		assertEquals( story_list.size() , 0 );
		IProject testProject = this.CP.getProjectList().get(0);
		//製造三筆mock data
		...
		/**真正所要測試的方法*/
		story_list = this.MSservice.getStorys( testProject.getName() );
		File actualFile = this.MSservice.getAttachFile(Long.toString(1));
		this.MSservice.closeConnect();
		//test case:測試加完三筆資料後 story_list size 為 3
		assertEquals( story_list.size() , 3 );
	...
	/**
	 * 主要測試getAttachFile方法中
	 * m_attachFileService.getAttachFile(fileID);對getAttachFile造成的效果是否有效
	 */
