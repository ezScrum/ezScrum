...
public class MantisServiceTest extends TestCase {
	...
	/**
	 * 主要測試getIssue方法中 m_attachFileService.initAttachFile(issue);對getIssue造成的效果是否有效
	 */
	public void testGetIssue_AboutAttachFile() {
		IIssue story = new Issue();// new issue data
		story.setIssueID(1);
		story.setSummary("Story_Name_One");
		story.setDescription("Story_Desc_One");
		story.setProjectID(this.CP.getProjectList().get(0).getName());
		...
		long storyId = this.MSservice.newIssue(story);
		/** 真正所要測試的方法 */
		IIssue storyOne = this.MSservice.getIssue(storyId);// 從mock data中利用storyID抓出story
		// test start
		...
		// test case : one file p.s.名稱、路徑因為上傳後會改變所以無法比較
		AttachFileObject attachFileObject = this.MSservice.getAttachFile(1);
		this.MSservice.closeConnect();// release connection
		assertEquals(storyOne.getAttachFile().size(), 1);// size shall be 1
		// 比對檔案大小是否一致! 原本用 getTotalSpace() 並非回傳檔案大小
		assertEquals(expectedFile.length(), attachFileObject.length());// space shall be equal
	}

	/**
	 * 主要測試getIssues方法中 p.s.這裡是複數s m_attachFileService.initAttachFile(issue);對getIssues造成的效果是否有效
	 */
	public void testGetIssues_AboutAttachFile() {
		List<IIssue> story_list = new LinkedList<IIssue>();
		IProject testProject = this.CP.getProjectList().get(0);
		// test case:測試初始狀態 story_list size 為 0
		...
		for (int i = 0; i < 3; i++) {
			...
			this.MSservice.newIssue(story);
			if (i == 1) {// 第二筆資料附加測試檔案
				this.MSservice.addAttachFile(story.getIssueID(), expectedFile1);// 將檔案附加上去
			}
		}
		/** 真正所要測試的方法 */
		story_list = Arrays.asList(this.MSservice.getIssues(testProject.getName(), ScrumEnum.STORY_ISSUE_TYPE));// refresh dirty story
		File actualFile = this.MSservice.getAttachFile(Long.toString(1));
		this.MSservice.closeConnect();
		// test case:測試加完三筆資料後 story_list size 為 3
		assertEquals(story_list.size(), 3);
		// test case:第一個story不存在任何檔案
		assertEquals(story_list.get(0).getAttachFile().size(), 0);
		// test case:第二個story存在一個測試檔案檔案
		assertEquals(story_list.get(1).getAttachFile().size(), 1);
		// 比對檔案大小是否一致! 原本用 getTotalSpace() 並非回傳檔案大小
		assertEquals(expectedFile1.length(), actualFile.length());// space shall be equal
	}

	/**
	 * 主要測試getStorys方法中 m_attachFileService.initAttachFile(story);對getStorys造成的效果是否有效
	 */
	public void testGetStorys_AboutAttachFile() {
		List<IStory> storyList = new ArrayList<IStory>();
		// test case:測試初始狀態 story_list size 為 0
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i == 1) {// 第二筆資料附加測試檔案
				this.MSservice.addAttachFile(story.getIssueID(), expectedFile1);// 將檔案附加上去
				story = this.MSservice.getIssue(story.getIssueID());// refresh dirty story
			}
		}
		/** 真正所要測試的方法 */
		storyList = this.MSservice.getStorys(testProject.getName());
		AttachFileObject attachFileObject = this.MSservice.getAttachFile(1);
		this.MSservice.closeConnect();
		// test case:測試加完三筆資料後 story_list size 為 3
		assertEquals(storyList.size(), 3);
		// test case:第一個story不存在任何檔案
		assertEquals(storyList.get(0).getAttachFile().size(), 0);
		// test case:第二個story存在一個測試檔案檔案
		assertEquals(storyList.get(1).getAttachFile().size(), 1);
		// 比對檔案大小是否一致! 原本用 getTotalSpace() 並非回傳檔案大小
		assertEquals(expectedFile1.length(), attachFileObject.length());// space shall be equal
	}

	/**
	 * 主要測試getAttachFile方法中 m_attachFileService.getAttachFile(fileID);對getAttachFile造成的效果是否有效
	 */
	public void testGetAttachFile_AboutAttachFile() {
		...
		this.MSservice.closeConnect();
		// 比對檔案大小是否一致! 原本用 getTotalSpace() 並非回傳檔案大小
		assertEquals(expectedFile.length(), actualFile.length());// space shall be equal
	}
